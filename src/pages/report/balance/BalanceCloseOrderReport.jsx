...
const BalanceCloseOrderReport = () => {
  ...
  const { isFetching } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}balanceCloseReport`,
        {
          headers: {
  ...
  const handleExcelExport = async () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `Balance-Close-Order-Report-${dayjs().format('DD-MM-YYYY')}.xlsx`;
      link.click();
              ...
              {reportData.length > 0 ? (
                <div>
                  <div ref={containerRef} className="md:overflow-x-auto">
                    <div className="p-4">
                      <h1 className="text-2xl font-bold text-center">Balance Close Order Report</h1>
                    </div>
